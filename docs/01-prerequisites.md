...
| Red Hat Connectivity Link (`rhcl-operator`) ^1^ | `openshift-operators` | API gateway policies, authentication (Authorino) and rate limiting (Limitador) for MaaS endpoints |
...
After the GPU operator CSVs have succeeded (see [Verification](#Verification) below), create the operand instances that configure NFD and the GPU driver stack.
...
| --- | --- |
| `AWS`, `Azure`, `GCP` (cloud) | **Skip to [Verification](#Verification) below** — no extra configuration needed. |
| `None`, `BareMetal`, `OpenStack` | **Continue with the steps below** — MetalLB and a passthrough Route are required. |
...
### Duplicate OperatorGroup after GUI + script install

If OpenShift AI was installed from the **OperatorHub GUI** before running this guide's Phase 1, verify only one `OperatorGroup` exists in `redhat-ods-operator`. A second group causes `rhods-operator` CSV `TooManyOperatorGroups`. See [Phase 4 troubleshooting](./04-rhoai-config.md#troubleshooting).

### RHOAI 3.5

This guide targets RHOAI 3.4 subscriptions; RHOAI 3.5 uses the same operator namespace and `stable-3.x` channel. `setup-maas.sh` applies version-specific DSC and dashboard patches when it detects 3.5+.
...
Proceed to [Phase 2: Platform Configuration](./02-platform-config.md).
