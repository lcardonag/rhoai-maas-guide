...
*Root cause:* BBR/payload-processing rewrites `:path` to `/v1/chat/completions`, dropping `/v1/projects/<uuid>/inference`. Gateway API `URLRewrite` alone cannot restore it. `RequestHeaderModifier` Host / `x-ibm-rhai-prefix` patches are applied at *router* time, so a Lua filter after `ext_proc.bbr` must *not* rely on those headers.

*Fix:* EnvoyFilter `ibm-rhai-upstream-path-prefix` in `openshift-ingress`, targeting Gateway `maas-default-gateway`. Lua inserted *after* `ext_proc.bbr` detects IBM models via `routeName` / `X-Gateway-Model-Name` / original path `/llm/<model>/`, then sets `:path` to `prefix + "/" + rest`.
...
* The MaaS reconciler may overwrite HTTPRoute header patches; this EnvoyFilter is the durable fix.
* Import / Compact MaaS still use `--path-prefix` / `compact-maas/upstream-path-prefix` for discovery and URLRewrite heal; keep that annotation, and *also* apply this filter.
...
Direct IBM calls succeed; gateway calls with a MaaS key return empty 404. BBR normalizes to `/v1/chat/completions` and drops `/v1/projects/<uuid>/inference`. Apply <<ibm-rhai-envoyfilter>>. Do not expect Host/`x-ibm-rhai-prefix` HTTPRoute patches alone to fix it after BBR.
