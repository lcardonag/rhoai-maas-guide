...
| `--from-phase <N>`
| Start from phase N (0-10), skipping earlier phases
| `0`
...
| `--with-litemaas`
| Also run Phase 9 (LiteMaaS + LiteLLM PoC; sibling `litemaas-rhoai`)
|

| `--with-maas-console`
| Also run Phase 10 (RHOAI MaaS Console; sibling `rhoai-maas-console`)
|
...
| 9
| LiteMaaS + LiteLLM (optional) - sibling install into `litemaas`
| 5-15 min

| 10
| MaaS Console (optional) - thin native UI into `rhoai-maas-console`
| 5-15 min
|===
...
See xref:09-optional-guis.adoc[Optional GUIs] for sibling repo paths and when to choose each.
...
Then register OpenAI-compatible SaaS or IBM RHAI models per xref:08-external-models.adoc[Phase 8: External Models] (console Admin *Upstream path prefix*, or YAML templates under `manifests/08-external-models/openai-compatible-prefixed/`).
